...
const Dashboard = ({friendsWithLastMessage,session}:TestProps) => {
  ...
  useEffect(()=>{
    ...
    pusherClient.bind('dashboard',dashboard)
    return ()=>{
        pusherClient.unsubscribe(toPusherKey(`chat:${session.user.id}`))
        pusherClient.unbind('dashboard',dashboard)
    }
